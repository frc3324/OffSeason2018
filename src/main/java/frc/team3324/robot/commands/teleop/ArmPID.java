...
public class ArmPID extends Command {
    ...
    public ArmPID() {
        mPID = new BasicPID(1024, 1.032, 0.251, 2.41, 21);
    } // Ticks per rev, mass(kg), length(m), stall torque (Nm), gear reduction.
    ...
    protected void execute() {
        ...
        if (Math.abs(joystickVal) > 0.1) {
            ...
            goal = 0.0;
            move = mPID.getPID(goal, testArmEncoder) + mPID.eDynamic(testArmEncoder, Math.toRadians(90)) -
                   mPID.eStat(testArmEncoder);
